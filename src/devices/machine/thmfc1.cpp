...
// THMFC1 floppy drive controller
...
{
	if(m_cmd2 & C2_DRS0)
		m_cur_floppy = m_floppy[0];
	else if(m_cmd2 & C2_DRS1)
		m_cur_floppy = m_floppy[1];
	else
		m_cur_floppy = nullptr;
}
...
void thmfc1_device::cmd2_w(u8 data)
{
	...
	if(m_cur_floppy) {
		if((prev & C2_MTON) && !(m_cmd2 & C2_MTON))
			m_timer_motoroff->adjust(attotime::from_seconds(60));
		if(m_cmd2 & C2_MTON) {
...
bool thmfc1_device::read_one_bit(u64 limit, u64 &next_flux_change)
{
	...
	static int count = -1;
	count++;
	if ((m_shift_reg && m_bit_counter == 0xf)|| (count & 0xfff) == 0) {
		if (m_cur_floppy) {
			static double pp = -1;
			if (m_cur_floppy->get_pos() != pp) {
				LOGSHIFT("floppy position=%.6f\n", m_cur_floppy->get_pos());
				pp = m_cur_floppy->get_pos();
			}
		} else
			LOGSHIFT("no floppy\n");
...
bool thmfc1_device::write_one_bit(u64 limit)
{
	...
	static double pp = -1;
	if (m_cur_floppy->get_pos() != pp)
		LOGSHIFT("floppy position=%.6f\n", m_cur_floppy->get_pos()); pp = m_cur_floppy->get_pos();
	if (m_bit_counter == 0)
